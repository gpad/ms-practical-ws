...
import { AggregateVersion } from "../infra/aggregate"
...
export class UserRepository {
  ...
  async save(user: User, trace: DomainTrace, logger: Logger) {
    ...
    // emit the events
    logger.info("Emitting all events from successful write to database")
    const eventsToPublish = user.commitEvents()
    for (let index = 0; index < eventsToPublish.length; index++) {
      const event = eventsToPublish[index]
      const version = new AggregateVersion(user.version, index)
      await this.eventBus.emit(event.enrich({ trace, version }))
    }
